 ...
 * TODO: писалось на скорую руку, есть куча ненужных логов (в логи /tmp/far2*.log попадают введенные вами пароли!!!) и т.п. (пока все на этапе разработки) + нужно фильтровать tar.gz формат, поскольку сейчас у этого формата поддержка как в винде, что не есть хорошо - лучше обрабатывать "старым" линуксовым кодом этот формат
# far2l
