 ...
 * вдруг выяснилось, что far2l не поддерживает запароленные 7z архивы (в целом понятно почему - в исходниках только Си шные файлы для распаковки)
 * в far2l полностью перекочевал 7z функционал от 21.07 версии архиватора и far2l более не нуждается в отдельно утсановленном p7zip-full (7z бинарнике), со всеми поддерживаемыми им форматами файлов и т.п.
 * добавлена поддержка симлинков и хардлинков по дефолту (можно вернуть все назад в настройках плагина) + патч 7z (версия 21.07 более настороженно работает с симлинками)
 * TODO: писалось на скорую руку, есть куча ненужных логов /tmp/far2*.log
 * TODO: добавить в CMakeLists.txt сборку ускоряющего кода на ассемблере (хотя это усложнит саму сборку - 7z использует [`jwasm`](https://www.japheth.de/JWasm.html)), которого нет в пакетах linux, но легко собирается
# far2l
