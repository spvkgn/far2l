 ...
 * после  [`коммита`](https://github.com/VPROFi/far2l/commit/ea4a060e6139704066dd5934871a956e0a386736) в far2l полностью перекочевал 7z функционал от 21.07 версии архиватора и far2l более не нуждается в отдельно утсановленном p7zip-full (7z бинарнике), со всеми поддерживаемыми им форматами файлов и т.п.
 * TODO: писалось на скорую руку, есть куча ненужных логов (в логи /tmp/far2*.log попадают введенные вами пароли!!!) и т.п. (пока все на этапе разработки но собирается и можно потестировать) + нужно фильтровать tar.gz формат, поскольку сейчас у этого формата поддержка как в винде, что не есть хорошо - лучше обрабатывать "старым" линуксовым кодом этот формат
# far2l
